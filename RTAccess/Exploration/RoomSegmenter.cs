...
/// edges between them out. Deliberately BCL-pure (no Unity, no Kingmaker) so it compiles by link into
/// <c>tests/RTAccess.Tests.csproj</c> alongside the graph core — <see cref="RoomMap"/> keeps everything that needs
/// the engine (reading the live A* grid, world positions, naming, fog, speech) and this file keeps everything that
/// is arithmetic. Anything added here MUST stay BCL-pure or the test project stops building (that's the point).
///
...
/// </summary>
internal static class RoomSegmenter
{
