...
namespace Access.Core.Graph
{
    ...
    public sealed class KeyGraph
    {
        ...
        // Public (not internal) because Access.Core is a real assembly boundary now: navigators live in the
        // per-game mod and legitimately need to resolve a landing node against a SPECIFIC render+state pair
        // rather than the instance's current one (seating a declared initial focus stop before the differ
        // announces). Widening delta vs the WrathAccess upstream — additive, safe to sync back.
