...
/// session behaves like a REPL. Ported from WrathAccess (originally TangledeepAccess). DEBUG-only.
///
...
internal sealed class CSharpEvaluator
{
    ...
    private void Initialize()
    {
        ...
        try
        {
            // Mod-specific conveniences so eval'd probes can use bare names (DevApi.Say, Speaker.Speak,
            // the existing a11y readers …). Separate Run so a resolution hiccup here can't drop the
            // engine-generic usings above.
            _evaluator.Run(
                "using RTAccess; using RTAccess.Dev; using RTAccess.Speech; using RTAccess.Accessibility;");
        }
        catch { /* mod namespaces should always resolve; tolerate it if they somehow don't */ }
    }
