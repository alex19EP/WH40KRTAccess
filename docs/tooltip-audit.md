...
- **Mod site:** `src/RTAccess/Screens/CharacterInfoScreen.cs:324-332` — `GetSoulMarkInfo` discards `rankThresholds` / `maxValue` / `currentValue` into `out _`; the label is direction name + rank tier name only.
...
- **Mod site:** `src/RTAccess/Screens/CharGen/AttributesPhaseContent.cs:21-37` (points header + 9 stat rows only) and `src/RTAccess/Screens/CharGen/SummaryPhaseContent.cs:52-93` (level + careers + the 9 attributes only).
...
- **Mod site:** `src/RTAccess/UI/DialogNodes.cs:58-66` (`OpenAnswerTooltip`) + `:80-98` (`AnswerText`). The mined links come from `UIConstsExtensions.GetAnswerFormattedString`, which emits only the skill-check DC keys and `UIDialogExchangeLinkFormat` (`UIConstsExtensions.cs:21-53`) — never the conditions anchor.
...
- **Mod site:** `src/RTAccess/Screens/BookEventScreen.cs:132-149` (`PassageLines` reads `cue.RawText` only) + `:56-63` (`SpeakLine`).
...
- **Mod site:** `src/RTAccess/Screens/ShipCustomizationScreen.cs:733-741` (`PostAbilityNode` label). Mod-wide grep for `HasDuration` / `UltimateDuration`: zero hits.
...
- **Mod sites:** `src/RTAccess/Screens/CharacterInfoScreen.cs:428-430` (wounds line, bare `GraphNodes.Text`); `src/RTAccess/Screens/InventoryScreen.cs:143-144` (character header, `ViewedCharacter.HeaderLine`; `BuildStatSection` is called with `withWounds:false` at `:106`/`:109`); `src/RTAccess/Screens/InGameScreen.cs:359-369` (HUD party roster row vtable — no `OnTooltip`).
...
- **Mod sites:** `src/RTAccess/Screens/CharacterInfoScreen.cs:277-287`; `src/RTAccess/Screens/RespecScreen.cs:96-106` (this second site is low on its own).
...
- **Mod sites:** `src/RTAccess/Screens/CharacterInfoScreen.cs:151-158` (`AllCareerPaths` tuples as bare `GraphNodes.Text`); `src/RTAccess/Screens/LevelUpScreen.cs:230-231` (`BeginStop("head")` → `ProgressHeader(cp)`) — the second is low on its own.
...
- **Mod site:** `src/RTAccess/Screens/InventoryScreen.cs:403-409` — three `StatLine` calls with no tooltip argument.
...
- **Mod site:** `src/RTAccess/Screens/CharacterInfoScreen.cs:313-334` — `BuildBiography` declares the three soul-mark rows and the shift/story lists, no conviction node.
...
- **Mod site:** `src/RTAccess/Screens/InventoryScreen.cs:365-367` — group label is `prefix + weapon.Name` only.
...
- **Mod site:** `src/RTAccess/UI/ItemNodes.cs:56-70` — `ItemLabel` reads notable / `!CanUse` / grade / count / charges / favourite, never `IsTrash` or `ItemStatus`.
...
- **Mod sites:** `src/RTAccess/UI/ItemNodes.cs:642` (`ItemRow.OnTooltip` → `OpenTemplate(…, OwnTemplate(slot))`; reached by `LootItem` `:84` / `LootScreen.cs:148` and `StashItem` `:153` / `PlayerChestScreen.cs:87`); `src/RTAccess/UI/ItemNodes.cs:120-137` (`InsertedItem` — bare vtable, used at `OneSlotLootScreen.cs:77`); `src/RTAccess/Screens/VendorBuyScreen.cs:72-73`, `:87-88`, `:117-132`.
...
- **Mod site:** `src/RTAccess/Screens/VendorScreen.cs:238-240` — feeds `lv.NextLevelReputationPoints` into `vendor.tier_locked` unconditionally (`enGB/ui.json:870` "Reputation level {level}, locked, unlocks at {points} reputation").
...
- **Mod site:** `src/RTAccess/Screens/CharGen/SelectionPhaseContent.cs:89-94` — `SelectedDescription` returns `it.Feature?.Description`, used as the browse label of the `desc` stop (`:77`).
...
- **Mod site:** `src/RTAccess/Screens/CharGen/CareerPhaseContent.cs:62` — `if (!string.IsNullOrEmpty(SelectedDescription(items)))` gates the only node that carries `OnTooltip` (`:72-73`).
...
- **Mod site:** `src/RTAccess/Screens/CharGen/SummaryPhaseContent.cs:27-93` — every node is `GraphNodes.Text` / `GraphNodes.Button` with no `tooltip:` argument (`GraphNodes.cs:49-53`, `:84-107`); the `WizardScreen` footer (`:124-129`) and `CharGenNodes.RoadmapEntry` (`:102-126`) carry none either.
...
- **Mod site:** `src/RTAccess/Screens/CharGen/SummaryPhaseContent.cs:85-86` (stat rows are bare `GraphNodes.Text`).
...
- **Mod site:** `src/RTAccess/Screens/LevelUpScreen.cs:168-201` — `BuildCareerList` declares only the header text and the tier groups.
...
- **Mod site:** `src/RTAccess/UI/DialogNodes.cs:80-98` — `AnswerText` calls `GetAnswerFormattedString` unconditionally.
...
- **Mod site:** `src/RTAccess/Screens/DialogueScreen.cs:177-186` — history loop, `GraphNodes.Text` with no `OnTooltip`; `GraphNavigator.cs:330-338` then speaks `nav.no_tooltip`.
...
- **Mod site:** `src/RTAccess/Screens/LogReviewScreen.cs:135-147` — `LogLine` label is `Clean(msg.Message)` alone.
...
- **Mod site:** `src/RTAccess/Accessibility/HudGauges.cs:50-59` (`AppendMomentum` — a flat `Speak`, no graph node); `src/RTAccess/Screens/InGameScreen.cs:442-499` (`BuildCombat` has no momentum row).
...
- **Mod site:** `src/RTAccess/Accessibility/HudGauges.cs:106-111` — bare `Loc.T("gauge.necron_timer", new { value })`, no node, no tooltip.
...
- **Mod sites:** `src/RTAccess/Screens/ShipCustomizationScreen.cs:838` (`st:hull`), `:848` (`st:scrap`), `:853` (`st:xp`), `:857` (`st:armor`), `:865` (`st:shields`), `:875` (`st:speed`). All bare `GraphNodes.Text`, and none post-assigns `OnTooltip` (grep of `OnTooltip` in the file: only `:285, :345, :642, :722, :764, :816`).
...
- **Mod site:** `src/RTAccess/Screens/ShipCustomizationScreen.cs:733-741` (label) and `:778-780` (generic drill-in caption). Grep for `AttuneAbility` in the mod: only a doc-comment mention.
...
**Sites:** `src/RTAccess/Screens/CharacterInfoScreen.cs:149-150` (`charinfo.level`), `src/RTAccess/Screens/InventoryScreen.cs:153-155`.
...
**Site:** `src/RTAccess/Screens/CharacterInfoScreen.cs:369-370` (single story) and `:381-382` (multi-story body).
...
**Site:** `src/RTAccess/UI/CareerNodes.cs:29-37` (`SelectionGroup` returns a plain group vtable), used at `LevelUpScreen.cs:274`.
...
**Site:** `src/RTAccess/UI/CharGenNodes.cs:151` (OnTooltip = `TooltipTemplateStat` only) and `:162-170` (`StatValueText`).
...
**Site:** `src/RTAccess/Screens/WizardScreen.cs:127-129` passes no `tooltip:`; `src/RTAccess/Accessibility/CharGenAnnounce.cs:112-137` never reads `PhaseNextHint`.
...
**Sites:** `src/RTAccess/Screens/DialogueScreen.cs:264` and `src/RTAccess/Screens/BookEventScreen.cs:89`.
...
**Site:** `src/RTAccess/Screens/SpaceCombatScreen.cs:155` — `b.AddLabel(ControlId.Structural("posts:head:" + p), () => PostLine(captured))`; `GraphBuilder.AddLabel` (`:233-234`) emits a vtable with only an announcement.
...
**Site:** `src/RTAccess/UI/ActionBarNodes.cs:74` (live `ToggleState` announcement) and `:165-175` (reads only `IsSelected` + `MechanicActionBarSlot.IsActive()`). Mod-wide grep for `IsAlerted`: nothing.
...
**Site:** `src/RTAccess/Accessibility/HudGauges.cs:73-82` (`AppendVeil` — flat `Speak`, no node).
...
**Site:** `src/RTAccess/Screens/EncyclopediaScreen.cs:309-357` (`EmitPlanet` / `EmitAstropath`) and `:276-291` (`GlossaryEntryNode` mines Description only).
